  ...
  const fetchTableDataList_Fn = async () => {
    ...
    try {
      const response = await axios.post(
        url + 'index.php/webApi/Fund/fund_list', cread, // Empty body
        {
          headers: {
            'auth_key': auth_key,
          },
      ...
      console.log("Response Data Table:____", response?.data); // Log the actual response data
  ...
  return (
    ...
    <Spin
        ...
        <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            ...
            <h2 className='text-xl font-bold text-white'>Fund List tt</h2>
                <div className="w-full md:w-1/2">
                        ...
                        <div className="relative w-full">
                            ...
                            <input type="text" id="simple-search" className="bg-blue-950 border border-blue-900 text-white text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Search" required=""/>
                        </div>
                </div>
                ...
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead className="text-xs text-gray-700 uppercase bg-slate-200 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" className="px-4 py-3">Installment Amount </th>
                            <th scope="col" className="px-4 py-3">Allotment Order No.</th>

                            <th scope="col" className="px-4 py-3">Schematic Amount</th>
                            <th scope="col" className="px-4 py-3">Contigency Amount</th>
                            {/* <th scope="col" className="px-4 py-3">Head Account</th>
                    ...
                    {/* <td scope="row" className="px-4 py-3">{data?.sl_no}</td>   */}
                    <td scope="row" className="px-4 py-3">{data?.instl_amt}</td> 
                    <td scope="row" className="px-4 py-3"><a href={url + folderName + data?.allotment_no} target='_blank'>
                        <FilePdfOutlined style={{fontSize:22, color:'red'}} /></a></td>

                    <td scope="row" className="px-4 py-3">{data?.sch_amt}</td>
                    <td scope="row" className="px-4 py-3">{data?.cont_amt}</td>
                    ...
                    <td scope="row" className="px-4 py-3">
                        ...
                        onClick={() => {
                        navigate(`/home/fund_release/frlistedit/${data?.approval_no}`, {
                        state: {
