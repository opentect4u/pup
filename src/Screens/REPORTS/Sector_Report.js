...
const validationSchema = Yup.object({
  ...
  head_acc: Yup.string().required('Sector is Required'),
});
...
function Sector_Report() {
  ...
  const showReport = async (params) => {
    ...
    formData.append("sector_id", formik.values.head_acc);
    ...
    try {
      ...
      if (response?.data?.status > 0) {
        ...
        setPrintSecoundField(response?.data?.sector_name)
        // setShowForm(true);
      }
    ...
    // Download & Print Function Start 
      ...
      const printData = (para) => {
        ...
        printWindow.document.write(`
          ...
          <h2>${PrintPageName.Sector} Year: ${printYear} Sector: ${printSecoundField}</h2>
        `);
    ...
    <section className="bg-white p-5 dark:bg-gray-900">
      <div className="py-5 mx-auto w-full lg:py-5">
        <div className="grid grid-cols-1 gap-4">
          ...
          <Heading title={"Sector Wise  Report"} button="N" />
          ...
          <form onSubmit={formik.handleSubmit}>
            <div class="grid gap-4 sm:grid-cols-12 sm:gap-6">
              ...
              dark:focus:ring-blue-800 ml-auto"
                  onClick={() => {
                    navigate(`/home/report/sector-report-graph/${financeYear_submit == "" ? params?.id : financeYear_submit}`, {
                      state: {
