...
const HomeScreen = () => {
  ...
  const fetchProjectsList = async () => {
    ...
    await axios
      .post(`${ADDRESSES.FETCH_PROJECTS_LIST}`, formData, {
        headers: {
      ...
      .then(res => {
        if (res?.data?.status === 1) {
          ...
          setProjectsList(
            res?.data?.message?.map((item: any, _: any) => ({
    ...
    await axios
      ...
      .then(res => {
        ...
        if (res?.data?.status === 1) {
          ...
          Alert.alert(
            'Approval Photo',
            'Approval project photo uploaded successfully.',
          );
        } else {
          ToastAndroid.show(
        ...
        <View
          ...
          <View
            ...
            <ButtonPaper
              ...
              mode="contained"
              onPress={() =>
                Alert.alert('Remove', 'Do you want to remove the photo?', [
