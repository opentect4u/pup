            ...
            <ScrollView keyboardShouldPersistTaps='handled' scrollEnabled style={{
                ...
                <View style={{
                    ...
                    <View>
                        ...
                        <InputPaper label="Password" maxLength={50} leftIcon='lock-outline' keyboardType="default" secureTextEntry={visiblePassword} value={formData.password} onChangeText={(txt: any) => handleFormChange("password", txt)} customStyle={{
                            backgroundColor: theme.colors.background,
                        }} onPressRight={() => setVisiblePassword(!visiblePassword)} underlineColor={theme.colors.background} password />
                    </View>

                    <ButtonPaper disabled={!formData.username || !formData.password || isLoading} loading={isLoading} mode='contained' onPress={() => handleLogin(formData.username, formData.password)} style={{
                        marginTop: 15,
                        paddingVertical: 8
                    }}>{strings.buttonText}</ButtonPaper>
                </View>
            </ScrollView>
        </SafeAreaView>
    )
